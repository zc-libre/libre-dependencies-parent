 ...
 *
 * <p>This class provides a configured retry mechanism for MQTT connection and
 * messaging operations. It uses exponential backoff strategy to handle transient
 * failures gracefully and avoid overwhelming the MQTT broker.
 *
 * <p>Features:
 * <ul>
...
public class MqttRetryTemplate {
	...
	public <T> T execute(RetryCallback<T, Exception> callback) throws Exception {
		return retryTemplate.execute(callback, context -> {
			log.error("All retry attempts failed for MQTT operation after {} attempts",
		...
		return RetryTemplate.builder()
			...
			.withListener(new RetryListener() {
				...
				public <T, E extends Throwable> void onError(RetryContext context, RetryCallback<T, E> callback, Throwable throwable) {
					log.warn("MQTT operation failed (attempt {}/{}): {}",
						context.getRetryCount(), maxAttempts, throwable.getMessage());
				}

				@Override
				public <T, E extends Throwable> void close(RetryContext context, RetryCallback<T, E> callback, Throwable throwable) {
					log.error("MQTT retry operation failed permanently after {} attempts",
						context.getRetryCount());
				}
			})
			.build();
