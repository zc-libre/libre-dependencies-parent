...
/**
 * Enhanced MQTT message converter that uses Jackson for JSON
 * serialization/deserialization.
 *
 * <p>
 * This converter extends the default Paho message converter to provide JSON-based message
 * serialization, making it easier to work with complex object payloads in MQTT messages.
 * It handles both inbound and outbound message conversion automatically.
 *
 * <p>
 * Features:
 * <ul>
...
public class JacksonPahoMessageConverter extends DefaultPahoMessageConverter {
	 ...
	 *
	 * <p>
	 * This method attempts to deserialize the MQTT message payload as JSON. If JSON
	 * deserialization fails, it falls back to string conversion.
	 ...
	 *
	 * <p>
	 * This method serializes the message payload to JSON bytes. For string payloads, it
	 * uses UTF-8 encoding directly. For other object types, it uses JSON serialization.
	...
	protected byte[] messageToMqttBytes(Message<?> message) {
		...
		try {
			if (payload instanceof String stringPayload) {
				...
				log.debug("Converted object payload to JSON: {} -> {} characters", payload.getClass().getSimpleName(),
						jsonString.length());
